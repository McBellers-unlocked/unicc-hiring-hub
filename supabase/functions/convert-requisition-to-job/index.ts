...
${requisition.desirable_education || ''}
    ...
    // Populate job_competencies and Overall Assessment items
    const competenciesToInsert = [];
    const competencyDefs: Record<string, [string, string]> = {
    ...
    const hasSupervisor = requisition.management_competencies?.includes('Creating an empowering and motivating environment') || 
                         requisition.core_competencies?.includes('Creating an empowering and motivating environment');
    if (hasSupervisor) mandatoryComps.push('Creating an empowering and motivating environment');

    let orderIdx = 0;
    const addComp = (name: string) => {
      const def = competencyDefs[name];
      if (def) competenciesToInsert.push({
        job_id: newJob.id, competency_type: def[0], competency_name: name,
        description: def[1], weight: 1, order_index: orderIdx++
      });
    };

    mandatoryComps.forEach(addComp);
    [...(requisition.core_competencies || []), ...(requisition.management_competencies || []), ...(requisition.leadership_competencies || [])]
      .filter(c => !mandatoryComps.includes(c))
      .forEach(addComp);

    if (competenciesToInsert.length > 0) {
      await supabaseClient.from('job_competencies').insert(competenciesToInsert);
    }

    await supabaseClient.from('job_requirements').insert([
      { job_id: newJob.id, category: 'Overall Assessment', title: 'Overall fit to the organization', 
        description: 'Holistic assessment of cultural fit, values alignment, and long-term potential within the organization', weight: 1, order_index: 0 },
      { job_id: newJob.id, category: 'Overall Assessment', title: 'Potential',
        description: 'Assessment of growth potential and capacity to exceed role requirements', weight: 1, order_index: 1 }
    ]);

    await supabaseClient.from('job_requisitions').update({ converted_to_job_id: newJob.id, status: 'converted' }).eq('id', requisitionId);

    await supabaseClient.functions.invoke('create-audit-log', {
      body: { action: 'REQUISITION_CONVERTED', entity: 'job_requisitions', entityId: requisitionId, after: { converted_to_job_id: newJob.id } }
    })
