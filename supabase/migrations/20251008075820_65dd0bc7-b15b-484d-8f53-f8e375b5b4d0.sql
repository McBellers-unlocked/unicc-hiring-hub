-- Update existing user to HR Assistant if exists
UPDATE public.users 
SET role = 'HR Assistant'::user_role 
WHERE email = 'negyesi@unicc.org';

-- Update the handle_new_user function to assign HR Assistant role
...
BEGIN
  -- Check if this email should be an HR Assistant
  assigned_role := CASE 
    WHEN NEW.email = 'negyesi@unicc.org' THEN 'HR Assistant'::user_role
