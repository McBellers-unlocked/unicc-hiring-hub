...
-- Check if the secure policy already exists and drop it first to recreate it
DROP POLICY IF EXISTS "Candidates can view killer questions for job applications" ON public.killer_questions;

-- Create a more secure policy that only allows viewing killer questions in specific contexts
CREATE POLICY "Secure killer questions access" 
