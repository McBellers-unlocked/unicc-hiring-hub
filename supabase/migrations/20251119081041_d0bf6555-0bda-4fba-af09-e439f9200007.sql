-- Normalize chief approval mapping so Tima (CS) can approve DS/CSI requisitions
...
BEGIN
  ...
  -- Override division mapping for specific chiefs when needed
  -- Tima SONI (soni@unicc.org) is part of the DS/Cybersecurity leadership
  -- but currently has division "CS" in the users table. Treat her as DS chief
  -- for approval purposes so she can approve DS/CSI requisitions.
  IF v_user_email = 'soni@unicc.org' AND v_user_division = 'CS' THEN
    v_user_division := 'DS';
  END IF;
  ...
  IF v_req_unit IS NOT NULL THEN
    ...
    ELSIF v_req_unit LIKE '%OP%' THEN
      v_req_division := 'OP';
    ELSIF v_req_unit LIKE '%DS%' OR v_req_unit LIKE '%CSA%' OR v_req_unit LIKE '%CYBER%' OR v_req_unit LIKE '%CSI%' THEN
      -- Treat CSA / Cyber / CSI units as part of DS
      v_req_division := 'DS';
    END IF;
  END IF;
  ...
  IF v_user_email IN ('negyesi@unicc.org', 'soni@unicc.org', 'liuzzi@unicc.org', 'sethi@unicc.org', 'chauhan@unicc.org', 'grecuccio@unicc.org') THEN
    ...
    -- Other chiefs match their division (including Tima mapped to DS above)
