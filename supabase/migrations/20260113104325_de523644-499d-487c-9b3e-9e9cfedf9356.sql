...
BEGIN
  ...
  -- Check duty station diversity - NOW MANDATORY
  IF panel_members.station_diversity < 2 THEN
    issues := issues || jsonb_build_object(
      'type', 'error',
      'message', 'Panel must include members from different duty stations'
    );
  END IF;

  -- Check nationality diversity - NOW MANDATORY
  IF panel_members.nationality_diversity < 2 THEN
    issues := issues || jsonb_build_object(
      'type', 'error',
      'message', 'Panel must include members with different nationalities'
    );
  END IF;

  -- Check division diversity - info recommendation (excludes HR Rep) - STAYS AS RECOMMENDED
  ...
  -- Check required roles - Hiring Manager
  IF NOT EXISTS (
  ...
  -- Check required roles - HR Representative - NOW MANDATORY
  IF NOT EXISTS (
    SELECT 1 FROM job_interview_panel_members 
    ...
    issues := issues || jsonb_build_object(
      'type', 'error',
      'message', 'Panel must include an HR Representative'
    );
  END IF;
