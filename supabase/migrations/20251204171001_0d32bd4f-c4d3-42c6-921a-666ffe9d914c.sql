...
BEGIN
  ...
  -- Check division diversity - changed from error to info recommendation
  IF panel_members.division_diversity < 2 THEN
    warnings := warnings || jsonb_build_object(
      'type', 'info',
      'message', 'Selection policy recommends including panel members from another Division or not reporting to the same manager as the Hiring Manager'
    );
  END IF;
