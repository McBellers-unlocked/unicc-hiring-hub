...
interface STDACommentsProps {
  stdaId: string;
}

export const STDAComments = ({ stdaId }: STDACommentsProps) => {
  const queryClient = useQueryClient();
  const { user } = useAuth();
  const [newComment, setNewComment] = useState('');

  const { data: comments = [], isLoading } = useQuery({
    queryKey: ['stda-comments', stdaId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('hr_stda_comments')
  ...
  const addCommentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['stda-comments', stdaId] });
...
export const LastSTDACommentPreview = ({ stdaId }: { stdaId: string }) => {
