...
export const TransferComments = ({ transferId }: { transferId: string }) => {
  const queryClient = useQueryClient();
  const { user } = useAuth();
  const [newComment, setNewComment] = useState('');

  const { data: comments = [], isLoading } = useQuery({
    queryKey: ['transfer-comments', transferId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('hr_transfer_comments')
  ...
  const addCommentMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['transfer-comments', transferId] });
...
export const LastTransferCommentPreview = ({ transferId }: { transferId: string }) => {
  ...
  return <span className="text-xs text-muted-foreground italic">"{truncated}"</span>;
};
