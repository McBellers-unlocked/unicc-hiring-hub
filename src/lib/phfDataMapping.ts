...
export function createPHFDataFromProfile(profile: any): any {
  return {
    personalDetails: {
      ...
      maidenName: profile.maiden_name_detailed || profile.maiden_name || '',
      sex: profile.gender || 'Male',
      dateOfBirth: profile.date_of_birth ? new Date(profile.date_of_birth) : new Date(),
      placeOfBirth: profile.place_of_birth_detailed || profile.place_of_birth || '',
      countryOfBirth: profile.country_of_birth_detailed || profile.country_of_birth || '',
      presentNationality: profile.present_nationality_detailed || profile.present_nationality || '',
      nationalityChanged: profile.nationality_changed || false,
      nationalityChangeDetails: profile.nationality_change_details_detailed || profile.nationality_change_details || '',
      maritalStatus: profile.marital_status_detailed || profile.marital_status || 'Single',
