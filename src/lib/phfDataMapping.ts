...
export function createPHFDataFromProfile(profile: any): any {
  return {
    personalDetails: {
      ...
      maidenName: profile.maiden_name_detailed || '',
      sex: profile.gender || 'Male',
      dateOfBirth: profile.date_of_birth ? new Date(profile.date_of_birth) : new Date(),
      placeOfBirth: profile.place_of_birth_detailed || '',
      countryOfBirth: profile.country_of_birth_detailed || '',
      presentNationality: profile.present_nationality_detailed || '',
